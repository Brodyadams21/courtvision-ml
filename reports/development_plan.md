...
| 10 | FastAPI inference service | Partial / local complete | `courtvision.api` has `/health`, `/predict/shot`, and `/predict/shots`, Pydantic schemas, `ShotModelService`, configurable lazy/startup model loading, request ID and prediction metadata logging, and tests; Docker/API image and production deployment docs remain |
